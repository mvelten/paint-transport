...
difference() {
    ...
    for(x=[0:3])
        ...
        // bottle holes
    for(x=[3:4])
        translate([10 + margin_x + citadel_radius + x*margin_x + x*citadel_pot, citadel_radius + margin_y, 2])
            cylinder(r=citadel_radius, h=citadel_height);
    
    // holes to reduce material consumption
    for(x=[3:4])
        // @TODO remove magic numbers
        translate([5 +citadel_radius + x*margin_x + x*citadel_pot, citadel_radius + margin_y-50, 3.5])
            cube([15,100,18]);
};
